        ...
        if(req.session.user){
            const findUser = await userModel.findOne({username: req.session.user})
                if(findUser){
                    ...
                    return res.render('fail',{success: false, message: "The user not found!"})
                }
        }
        else{
            return res.render('fail',{success: false, message: "Please provide all details!"})
        }
    }
    catch(err){
        console.log('Error in Login middleware: ',err)
...
app.get('/enroll-students', (req, res)=>{
    ...
    try{
        ...
        if(req.session.user){
            ...
            if(studentName&&studentEmail){
                ...
                if(dataSave){
                    return res.render('success',{success: true, message: "Registration successfull!"})
                }
                else{
                    return res.render('fail',{success: false, message: "Student Registration failed!"})
                }
            }
            else{
                return res.render('fail',{success: false, message: "Please provide all details!"})
            ...
            return res.render('fail',{success: false, message: "Please login TO continue"})
        }
...
app.get('/logout', (req, res)=>{
    req.session.destroy()
    console.log('Request session data after loggin out: ',req.session)
        ...
        if(userName && userPassword){
            ...
            if(findUser){
               if(findUser.password===userPassword){
                    ...
                    console.log('Request session data after login: ',req.session)   
